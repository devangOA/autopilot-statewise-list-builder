...
const NY_PLACES = new Set(['dallas','fort worth','arlington','plano','garland','irving','frisco','mckinney','grand prairie','denton','mesquite','carrollton','richardson','lewisville','allen','flower mound','north richland hills','mansfield','euless','grapevine','bedford','cedar hill','wylie','keller','coppell','rowlett','desoto','little elm','prosper','southlake','colleyville','the colony','farmers branch','rockwall','burleson','haltom city','hurst','duncanville','lancaster','waxahachie','midlothian','weatherford','cleburne','houston','pasadena','pearland','sugar land','the woodlands','league city','baytown','conroe','missouri city','katy','spring','cypress','friendswood','deer park','la porte','texas city','galveston','rosenberg','humble','tomball','stafford','bellaire','kingwood','atascocita','richmond','fulshear','channelview','austin','round rock','cedar park','georgetown','pflugerville','san marcos','leander','kyle','buda','hutto','lakeway','dripping springs','bastrop','san antonio','new braunfels','schertz','seguin','cibolo','universal city','boerne','converse','live oak','selma','helotes','el paso','socorro','horizon city','mcallen','brownsville','harlingen','edinburg','pharr','mission','weslaco','san benito','mercedes','corpus christi','portland','kingsville','robstown','beaumont','port arthur','orange','nederland','lumberton','waco','temple','killeen','belton','harker heights','college station','bryan','huntsville','brenham','lubbock','amarillo','midland','odessa','san angelo','abilene','big spring','wichita falls','plainview','tyler','longview','marshall','nacogdoches','lufkin','texarkana','paris','sherman','denison','greenville','palestine','laredo','del rio','eagle pass','uvalde','victoria','fredericksburg','kerrville','marble falls']);
...
export function detectCity(text) {
  const t = String(text || '').replace(/\s+/g, ' ');
  const re = /([A-Za-z][A-Za-z.'’\- ]{2,60}?),\s*(?:TX|Texas)\b/g;
...
export function detectAddresses(text) {
  ...
  const re = new RegExp(
    `(\\d{1,6}[A-Za-z]?\\s+[A-Za-z0-9.'\`\\-\\s]{2,40}?${STREET_SUFFIX}\\.?)` + // street
      ...
      `\\s*,\\s*(?:TX|Texas)\\b\\s*(\\d{5})?`, // state + optional zip
    'g',
  );
  const seen = new Map();
  let m;
...
const NY_AREA_CODES = /\(?(210|214|254|281|325|346|361|409|430|432|469|512|682|713|737|806|817|830|832|903|915|936|940|945|956|972|979)\)?[)\s.-]{1,3}\d{3}[\s.-]?\d{4}/;
...
export function detectNyEvidence(text) {
  const t = String(text || '').replace(/\s+/g, ' ');
  if (/,\s*(?:TX|Texas)\b/i.test(t)) return 'address';
  if (/\bTX\s+7[5-9]\d{3}\b/.test(t)) return 'zip';
  if (NY_AREA_CODES.test(t)) return 'phone';
  if (/\bTexas\b/i.test(t)) return 'mention';
  return '';
}
