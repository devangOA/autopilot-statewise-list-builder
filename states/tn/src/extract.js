...
const NY_PLACES = new Set(['nashville','franklin','brentwood','murfreesboro','smyrna','la vergne','hendersonville','gallatin','mount juliet','lebanon','spring hill','columbia','nolensville','thompson\'s station','goodlettsville','madison','antioch','dickson','fairview','white house','portland','springfield','clarksville','oak grove','memphis','germantown','collierville','bartlett','cordova','millington','lakeland','arlington','jackson','dyersburg','union city','martin','brownsville','covington','knoxville','farragut','maryville','alcoa','oak ridge','sevierville','pigeon forge','gatlinburg','morristown','clinton','powell','lenoir city','loudon','seymour','jefferson city','dandridge','newport','chattanooga','cleveland','east ridge','red bank','hixson','collegedale','ooltewah','soddy-daisy','signal mountain','dayton','athens','mcminnville','tullahoma','manchester','shelbyville','winchester','lynchburg','lawrenceburg','pulaski','lewisburg','fayetteville','johnson city','kingsport','bristol','greeneville','elizabethton','erwin','jonesborough','cookeville','crossville','livingston','sparta']);
...
export function detectCity(text) {
  const t = String(text || '').replace(/\s+/g, ' ');
  const re = /([A-Za-z][A-Za-z.'’\- ]{2,60}?),\s*(?:TN|Tennessee)\b/g;
...
function normalizeCity(s) {
  ...
  // (no metropolis alias for Tennessee)
...
export function detectAddresses(text) {
  ...
  const re = new RegExp(
    `(\\d{1,6}[A-Za-z]?\\s+[A-Za-z0-9.'\`\\-\\s]{2,40}?${STREET_SUFFIX}\\.?)` + // street
      ...
      `\\s*,\\s*(?:TN|Tennessee)\\b\\s*(\\d{5})?`, // state + optional zip
    'g',
  );
  const seen = new Map();
  let m;
...
const NY_AREA_CODES = /\(?(423|615|629|731|865|901|931)\)?[)\s.-]{1,3}\d{3}[\s.-]?\d{4}/;
...
export function detectNyEvidence(text) {
  const t = String(text || '').replace(/\s+/g, ' ');
  if (/,\s*(?:TN|Tennessee)\b/i.test(t)) return 'address';
  if (/\bTN\s+3[78]\d{3}\b/.test(t)) return 'zip';
  if (NY_AREA_CODES.test(t)) return 'phone';
  if (/\bTennessee\b/i.test(t)) return 'mention';
  return '';
}
