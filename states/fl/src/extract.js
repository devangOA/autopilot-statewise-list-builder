...
const NY_PLACES = new Set(['miami','miami beach','miami gardens','hialeah','coral gables','doral','kendall','homestead','aventura','north miami','north miami beach','cutler bay','palmetto bay','pinecrest','sweetwater','miami lakes','key biscayne','fort lauderdale','hollywood','pembroke pines','miramar','coral springs','pompano beach','davie','plantation','sunrise','deerfield beach','weston','tamarac','margate','coconut creek','lauderhill','hallandale beach','cooper city','parkland','west palm beach','boca raton','delray beach','boynton beach','jupiter','palm beach gardens','wellington','lake worth','royal palm beach','greenacres','palm beach','riviera beach','juno beach','port st. lucie','stuart','vero beach','jensen beach','fort pierce','sebastian','palm city','naples','bonita springs','estero','fort myers','cape coral','marco island','lehigh acres','punta gorda','port charlotte','sarasota','bradenton','venice','lakewood ranch','palmetto','north port','osprey','nokomis','tampa','st. petersburg','clearwater','brandon','riverview','wesley chapel','palm harbor','largo','pinellas park','dunedin','safety harbor','oldsmar','land o lakes','lutz','valrico','apollo beach','new port richey','trinity','tarpon springs','seminole','lakeland','winter haven','plant city','bartow','auburndale','davenport','haines city','orlando','winter park','kissimmee','altamonte springs','sanford','lake mary','oviedo','winter garden','clermont','apopka','maitland','casselberry','longwood','ocoee','windermere','st. cloud','winter springs','ocala','the villages','gainesville','lady lake','leesburg','mount dora','tavares','eustis','jacksonville','orange park','st. augustine','ponte vedra beach','palm coast','fleming island','fernandina beach','st. johns','daytona beach','port orange','deltona','deland','new smyrna beach','ormond beach','melbourne','palm bay','titusville','cocoa','cocoa beach','rockledge','merritt island','satellite beach','tallahassee','panama city','panama city beach','pensacola','fort walton beach','destin','navarre','niceville','crestview','gulf breeze','key west','key largo','marathon','islamorada']);
...
export function detectCity(text) {
  const t = String(text || '').replace(/\s+/g, ' ');
  const re = /([A-Za-z][A-Za-z.'’\- ]{2,60}?),\s*(?:FL|Florida)\b/g;
...
function normalizeCity(s) {
  ...
  // (no metropolis alias for Florida)
...
export function detectAddresses(text) {
  ...
  const re = new RegExp(
    `(\\d{1,6}[A-Za-z]?\\s+[A-Za-z0-9.'\`\\-\\s]{2,40}?${STREET_SUFFIX}\\.?)` + // street
      ...
      `\\s*,\\s*(?:FL|Florida)\\b\\s*(\\d{5})?`, // state + optional zip
    'g',
  );
  const seen = new Map();
  let m;
...
const NY_AREA_CODES = /\(?(239|305|321|352|386|407|448|561|656|689|727|754|772|786|813|850|863|904|941|954)\)?[)\s.-]{1,3}\d{3}[\s.-]?\d{4}/;
...
export function detectNyEvidence(text) {
  const t = String(text || '').replace(/\s+/g, ' ');
  if (/,\s*(?:FL|Florida)\b/i.test(t)) return 'address';
  if (/\bFL\s+3[234]\d{3}\b/.test(t)) return 'zip';
  if (NY_AREA_CODES.test(t)) return 'phone';
  if (/\bFlorida\b/i.test(t)) return 'mention';
  return '';
}
